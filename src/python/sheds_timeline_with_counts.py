...
def create_timeline_with_counts(df, output_base='sheds_timeline_counts', 
    ...
    ax.set_yticklabels([CATEGORY_LABELS.get(c, c) for c in ordered_cats], fontsize=12)
    

    ax.set_xticks(range(len(year_cols)))
    ax.set_xticklabels(year_cols, fontsize=12)
    ...
    ax.legend(handles=legend_elements, loc='upper center', 
             bbox_to_anchor=(0.5, -0.08), ncol=5, fontsize=12, frameon=False)
